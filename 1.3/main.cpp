...
void td(string x){
    ...
    if(money<10){
    	cout<<"你没钱了，快去卖树赚钱"<<endl;
			...
			s[ans].buy=rand()%50+1;
			cout<<"本东西价值:"<<s[ans].buy;
		}
	}
	else if(x=="/have"){
		money+=tree;
		cout<<"你赚了"<<tree<<"元"<<endl;
		tree=0;
	}
	else if(x=="/rand"){
		srand(time(NULL));
    ...
    if(r==1){
		...
		for(int i=1;i<=ans;i++){
			cout<<"名字:"<<s[i].name<<" 价值:"<<s[i].buy<<endl;
		}
		cout<<"以上是你的所有东西"<<endl;
	}
	else if(x=="/clearbag"){
		...
		cout<<"11./make 用钱制作东西"<<endl;
		cout<<"12./have 砍了树赚钱"<<endl;
		cout<<"13./bag 查看自己的所有东西"<<endl;
		cout<<"14./clearbag 清空背包并获得钱"<<endl;
		cout<<"15./rand 物理模拟"<<endl;
		cout<<"/music 发出声音"<<endl;
		...
		cout<<"要什么主题?"<<endl<<"1.经典黑白"<<endl<<"2.黑客风"<<endl<<"3.蓝天白云"<<endl;
		int n;
		cin>>n;
		if(n==1){
			system("color 07");
